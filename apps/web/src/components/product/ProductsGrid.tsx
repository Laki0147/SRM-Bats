...
// Real product photography — cycled across the catalogue for angle variety.
const BAT_IMAGES = [
  '/bats/front.png',
  '/bats/cross.png',
  '/bats/side.png',
    ...
    >
      <Link href={`/products/${p.slug}`} className="block">
        ...
        >
          ...
          >
            <Image
              src={BAT_IMAGES[index % BAT_IMAGES.length]}
              alt={p.name}
              fill
            ...
            {discount > 0 && (
              <span
                className="absolute left-3 top-3 rounded-full px-2 py-1 font-body text-[10px] font-bold"
          ...
          <div className="p-4 pb-5">
            ...
            <div className="mb-2 flex items-center gap-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  className={`h-3 w-3 ${i < p.rating ? 'fill-[#8b5e3c] text-[#8b5e3c]' : 'text-[#a09588] opacity-25'}`}
                />
              ))}
            </div>
            ...
            <div className="mb-3 flex flex-wrap gap-1">
              {[p.grade, p.profile].map((tag) => (
                <span
                  key={tag}
                  className="rounded-full px-2.5 py-[3px] font-body text-[10px] font-medium"
    ...
    <section className="px-6 py-12 lg:px-[52px]">
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <SlidersHorizontal className="h-4 w-4" style={{ color: '#8b5e3c' }} />
